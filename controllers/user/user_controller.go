...
func checkMemberInfoInDatabase(account, password string) bool {
	var exists bool
	err := services.GameDB.Raw(`SELECT CASE WHEN EXISTS (SELECT 1 FROM dbo.member_info WHERE memb___id COLLATE Latin1_General_CS_AS = ? AND password Latin1_General_CS_AS = ?) THEN 1 ELSE 0 END`, account, password).Scan(&exists).Error
	if err != nil {
