...
function WndDualGame_StartEffectGainHP(characterIndex, comboIndex, x, y, damage)
...
function WndDualGame_EndEffectGainHP(characterIndex, comboIndex)
	if characterIndex < 0 and characterIndex >= #tGainHPPosX then
		return
	end
...
function WndDualGame_RenderEffectGainHP(characterIndex, comboIndex, deltaTime)
	if characterIndex < 0 and characterIndex >= #tGainHPPosX then
		return
	end
...
function WndDualGame_StartEffectReflectDamage(characterIndex, comboIndex, x, y, damage)
...
function WndDualGame_EndEffectReflectDamage(characterIndex, comboIndex)
	if characterIndex < 0 and characterIndex >= #tReflectDamagePosX then
		return
	end
...
function WndDualGame_RenderEffectReflectDamage(characterIndex, comboIndex, deltaTime)
	if characterIndex < 0 and characterIndex >= #tReflectDamagePosX then
		return
	end
...
function WndDualGame_StartEffectGainSP(characterIndex, comboIndex, x, y, damage)
...
function WndDualGame_EndEffectGainSP(characterIndex, comboIndex)
	if characterIndex < 0 and characterIndex >= #tGainSPPosX then
		return
	end
...
function WndDualGame_RenderEffectGainSP(characterIndex, comboIndex, deltaTime)
	if characterIndex < 0 and characterIndex >= #tGainSPPosX then
		return
	end
...
function WndDualGame_InitPoisonEffect(index)
...
function WndDualGame_RenderPoisonEffect(index, x, y, damage, passTick)
...
----------------------------------------------------
function WndDualGame_RenderBackground(redPlayerIndex, bluePlayerIndex)
...
--------------------------------------------------------------------
function WndDualGame_StartGhost(downIndex, x, y, attackName1, attackName2)
	Common_StartGhost(downIndex, x, y, attackName1, attackName2)
end



function WndDualGame_EndGhost(downIndex)
	Common_EndGhost(downIndex)
end



function WndDualGame_RenderGhost(slot, x, y, deltaTime)
...
--------------------------------------------------------------------
function WndDualGame_RenderCharacter
			( deltatime
			, myslot
            , myteam
			, slot
...
-- 현재 나의 랭크로 사이즈를 정한다.
function WndDualGame_BeforeRenderRank(myRank)
...
function WndDualGame_RenderRank(slot, myslot, bone, bTeam, team, 
...
function WndDualGame_InitTeamState(deltaTime)
	...
	--[[
	if playerAlphaFlag == 0 then
		playerAlpha = playerAlpha + (deltaTime / 2)
	else
		playerAlpha = playerAlpha - (deltaTime / 2)
	end
	
	if playerAlpha >= 255 then
		playerAlpha = 255
		playerAlphaFlag = 1
...
function WndDualGame_NotTeamState(count)
...
function WndDualGame_TeamState(deltaTime, count, enemyType, name, state, kill, killed, disconnect, playerState, playerReadyFlag)
...
--	killed = 0
	...
	if killed > 0 then
		drawer:drawTexture("UIData/GameImage_dual.tga", posX+KILL_POS_X, posY, 21, 30, 618, 359, WIDE)
	end
end

-- 현재 플레이중인 유저 알려주기
function WndDualGame_NotifyCurrentPlayer(playerState)
	--[[
...
----------------------------------------------------------
function WndDualGame_RenderWinPlayer(deltaTime, name, winTeam)
	if winTeam == 0 then
...
mywindow = winMgr:createWindow("TaharezLook/StaticImage", "sj_WndDualGame_Effect_combo")
...
winMgr:getWindow("sj_WndDualGame_Effect_combo"):addController("CommonAlertOkBoxCtrl0", "StampEffect", "xscale", "Quintic_EaseIn", 700, 255, 3, true, false, 10)
winMgr:getWindow("sj_WndDualGame_Effect_combo"):addController("CommonAlertOkBoxCtrl0", "StampEffect", "yscale", "Quintic_EaseIn", 700, 255, 3, true, false, 10)
winMgr:getWindow("sj_WndDualGame_Effect_combo"):addController("CommonAlertOkBoxCtrl0", "StampEffect", "xscale", "Elastic_EaseOut", 150, 255, 5, true, false, 10)
winMgr:getWindow("sj_WndDualGame_Effect_combo"):addController("CommonAlertOkBoxCtrl0", "StampEffect", "yscale", "Elastic_EaseOut", 150, 255, 5, true, false, 10)
winMgr:getWindow("sj_WndDualGame_Effect_combo"):setAlign(8)

local g_ComboEffectEnable = false
local g_ComboEffectAlpha = 255
local g_ComboEffectTime = 0
function WndDualGame_ComboEffect(state)
	...
	winMgr:getWindow("sj_WndDualGame_Effect_combo"):setTexture("Enabled", "UIData/GameNewImage2.tga", 646, tComboEffectTexY[state])
	winMgr:getWindow("sj_WndDualGame_Effect_combo"):setVisible(true)
	winMgr:getWindow("sj_WndDualGame_Effect_combo"):activeMotion("StampEffect")
end
...
for i=1, #g_KoEffectEnable do
	mywindow = winMgr:createWindow("TaharezLook/StaticImage", "sj_WndDualGame_Effect_ko"..i)
	...
	winMgr:getWindow("sj_WndDualGame_Effect_ko"..i):addController("CommonAlertOkBoxCtrl0", "StampEffect", "xscale", "Quintic_EaseIn", 700, 255, 3, true, false, 10)
	winMgr:getWindow("sj_WndDualGame_Effect_ko"..i):addController("CommonAlertOkBoxCtrl0", "StampEffect", "yscale", "Quintic_EaseIn", 700, 255, 3, true, false, 10)
	winMgr:getWindow("sj_WndDualGame_Effect_ko"..i):addController("CommonAlertOkBoxCtrl0", "StampEffect", "xscale", "Elastic_EaseOut", 150, 255, 5, true, false, 10)
	winMgr:getWindow("sj_WndDualGame_Effect_ko"..i):addController("CommonAlertOkBoxCtrl0", "StampEffect", "yscale", "Elastic_EaseOut", 150, 255, 5, true, false, 10)
	winMgr:getWindow("sj_WndDualGame_Effect_ko"..i):setAlign(8)
end

function WndDualGame_KoEffect(koCount)
	if koCount <= 0 then
		return
	end
	...
	winMgr:getWindow("sj_WndDualGame_Effect_ko"..koCount):setTexture("Enabled", "UIData/GameNewImage2.tga", 0, g_KoEffectTexY[koCount])
	winMgr:getWindow("sj_WndDualGame_Effect_ko"..koCount):setVisible(true)
	winMgr:getWindow("sj_WndDualGame_Effect_ko"..koCount):activeMotion("StampEffect")
...
----------------------------------------------------------
function WndDualGame_RenderKillAndDead(bTeam, killEnemyType, deadEnemyType, count, killName, deadName)
...
function WndDualGame_RenderMission(deltaTime, ready, fight, bComplete, bSuccess0, bSuccess1, contents0, contents1, bLast1Min)
...
function WndDualGame_InitStartInfo()
	g_readySound = true
	g_fightSound = true
end
...
function WndDualGame_RenderBattleReady(deltaTime)
	
	--tReadyDelta[1] = tReadyDelta[1] + deltaTime*3
	--if tReadyDelta[1] >= 720 then
		drawer:drawTexture("UIData/Notice_up.tga", 284, 316, 456, 137, 589, 130, WIDETYPE_5)
	--else
...
function WndDualGame_CallFight()
	 g_fightTime = 0
	 g_fightAlpha = 255
	 g_battleAlpha = 255
end
function WndDualGame_RenderBattleFight(deltaTime)
--[[
...
--]]
	...
	drawer:drawTextureA("UIData/Notice_up.tga", 284, 316, 456, 137, 589, 0, g_fightAlpha, WIDETYPE_5)
...
----------------------------------------------------------
mywindow = winMgr:createWindow("TaharezLook/StaticImage", "sj_WndDualGame_eventImage")
...
mywindow:setWideType(5);
mywindow:setPosition(0, 80)
mywindow:setSize(1024, 99)
mywindow:setVisible(false)
...
function StartValentineEvent()
	winMgr:getWindow("sj_WndDualGame_eventImage"):setVisible(true)
	winMgr:getWindow("sj_WndDualGame_eventImage"):activeMotion("gameEvent")
end
...
function WndDualGame_RenderScore(deltaTime, min, sec, state, redWinCount, blueWinCount)
	...
	if g_ComboEffectEnable then
		...
		if g_ComboEffectTime >= 1200 then			
			...
			if g_ComboEffectAlpha <= 0 then
				...
				winMgr:getWindow("sj_WndDualGame_Effect_combo"):setVisible(false)
			end
			winMgr:getWindow("sj_WndDualGame_Effect_combo"):setAlpha(g_ComboEffectAlpha)
		end
	end
		...
		if g_KoEffectEnable[i] then
			...
			if g_KoEffectTime[i] >= 1200 then
				...
				if g_KoEffectAlpha[i] <= 0 then
					...
					if winMgr:getWindow("sj_WndDualGame_Effect_ko"..i) then
						winMgr:getWindow("sj_WndDualGame_Effect_ko"..i):setVisible(false)
					end
				end
				
				if winMgr:getWindow("sj_WndDualGame_Effect_ko"..i) then
					winMgr:getWindow("sj_WndDualGame_Effect_ko"..i):setAlpha(g_KoEffectAlpha[i])
				end
			end
		else
			g_KoEffectAlpha[i] = 0
...
local tTimeTexX = {["err"]=0, [0]=0, 127, 254, 381, 508, 635, 762, 889, 762, 889}
local tTimeTexY = {["err"]=0, [0]=588, 588, 588, 588, 588, 588, 588, 588, 776, 776}
function RenderIntroTime(sec)
	--DrawEachNumberWide("UIData/Notice_up.tga", sec, 8, 480, 300, 0, 402, 73, 98, 73, WIDETYPE_5)	-- 카운트 다운
	if sec < 10 then
		drawer:drawTexture("UIData/Notice_up.tga", 448, 290, 127, 188, tTimeTexX[sec], tTimeTexY[sec], WIDETYPE_5)
		drawer:drawTexture("UIData/Notice_up.tga", 0, 170, 1024, 88, 0, 500, WIDETYPE_5)
	end
end
...
function WndDualGame_ComboAndDamage(deltaTime, isCombo, currentCombo, isAccumulate, accumDamage, 
...
----------------------------------------------------
--[[
function WndDualGame_RenderStartItemSlot(bSlot0, bSlot1, bSlot2)
end
...
-- 아이템 그리기
function WndDualGame_RenderItem(bChanged, itemType_0, itemType_1, tick, CHANGED_TICK)
...
-- 아이템 먹었을 때 효과 그리기
function WndDualGame_RenderEffectGetItem(slot, state)
...
function WndDualGame_InitEffectGetItem()
...
function WndDualGame_RenderEndItemSlot()
...
-- 아이템 체인지 가능한 수
function WndDualGame_RenderSlotChangeCount(slotChangeCount)
	if gMyTeam == 0 then
...
-- 아이템을 사용했을 때 보여주기
function WndDualGame_ItemUseEffect(type, tick)
...
-- 아이템 그리기
--[[
function WndDualGame_RenderPropertyItem(itemType, myHP, myDiedTime)
...
function WndDualGame_RenderGameOver(deltaTime)
...
----------------------------------------------------------
mywindow = winMgr:createWindow("TaharezLook/StaticImage", "sj_WndDualGame_NotifyExceptE")
...
function WndDualGame_ShowExceptE()
	winMgr:getWindow("sj_WndDualGame_NotifyExceptE"):setVisible(true)
	winMgr:getWindow("sj_WndDualGame_NotifyExceptE"):setScaleWidth(400)
	winMgr:getWindow("sj_WndDualGame_NotifyExceptE"):setScaleHeight(400)
	winMgr:getWindow("sj_WndDualGame_NotifyExceptE"):clearControllerEvent("notifyExceptE")
	winMgr:getWindow("sj_WndDualGame_NotifyExceptE"):addController("notifyExceptE", "notifyExceptE", "alpha", "Sine_EaseInOut", 255, 0, 10, false, false, 10)
	winMgr:getWindow("sj_WndDualGame_NotifyExceptE"):activeMotion("notifyExceptE")
end

function WndDualGame_HideExceptE()
	winMgr:getWindow("sj_WndDualGame_NotifyExceptE"):setVisible(false)
end
...
function WndDualGame_ClearChatting()
...
function GetUserChat()
	for i=1, table.getn(tShowText) do
		WndDualGame_UserChattingSave(tShowText[i], tShowTextColor[i])
	end
end
...
------------------------------------------------
function WndDualGame_OnDrawBoolean(str_chat, px, py, chatBubbleType)
...
function WndDualGame_RenderLast3(deltaTime, effect, section)
	if section == 0 then
...
function WndDualGame_RenderLast1(deltaTime, effect, section)
	if section == 0 then
...
function WndDualGame_InitLast1MinData()
	g_last1MinEffect= 0
	g_last1MinTime	= 0
	g_last1Min		= true
	g_last1MinPos	= 0
...
function WndDualGame_RenderLast1Min(deltaTime)
...
function WndDualGame_InitLast10SecData()
	g_last10Effect	= 0
	g_last10SecTime	= 0
	g_last10Sec		= true
	g_last10SecPos	= 0
...
function WndDualGame_RenderLast10Sec(deltaTime)
...
function WndDualGame_UserBattleInfo_bestPlayer(bMe1, bMe2, bMe3, bMe4, bMe5, bMe6, bMe7, bMe8, 
...
function WndDualGame_UserBattleInfo_whoMeDown(index, clubImageName, level, ladder, name, style, koNum, showRevenge, promotion, attribute)
...
----------------------------------------------------
function WndDualGame_RenderHelp(bHelp)
--[[
	-- 도움말 버튼
...
----------------------------------------------------
function WndDualGame_NotifyPickupWeapon(weaponIndex, weaponPosX, weaponPosY)
...
----------------------------------------------------
function WndDualGame_NotifyPickupPropertyItem(weaponIndex, weaponPosX, weaponPosY)
...
----------------------------------------------------
function WndDualGame_Debug(i, count, hp, name, myIndex)
...
-- 디버그 2
function WndDualGame_Debug2(i, name, atkDamage, grabDamage, superEDamage, doubleAtkCount, teamAtkCount, evadeCount, comboCount, itemUseCount, whoMeDown)
	drawer:setTextColor(255, 255, 0, 255)
	drawer:setFont(g_STRING_FONT_GULIMCHE, 12)
	--[[
	local TEX_X = 190
...
function WndDualGame_ShowEmblemKey(LeftEmblem, RightEmblem)
...
----------------------------------------------------
quitwindow = winMgr:createWindow("TaharezLook/StaticImage", "sj_WndDualGame_exitBackWindow")
...
mywindow = winMgr:createWindow("TaharezLook/Button", "sj_WndDualGame_exitOkBtn")
...
mywindow:setAlpha(0)
mywindow:subscribeEvent("Clicked", "WndDualGame_QuitOK")
quitwindow:addChildWindow(mywindow)

mywindow = winMgr:createWindow("TaharezLook/Button", "sj_WndDualGame_exitCancelBtn")
...
mywindow:setSize(90, 27)
mywindow:setAlpha(0)
mywindow:subscribeEvent("Clicked", "WndDualGame_QuitCancel")
quitwindow:addChildWindow(mywindow)


function WndDualGame_SetEscape(before30sec)
...
function WndDualGame_Escape(bFlag, deltaTime)
	...
	winMgr:getWindow("sj_WndDualGame_exitBackWindow"):setAlpha(g_escapeAlpha)
	winMgr:getWindow("sj_WndDualGame_exitOkBtn"):setAlpha(g_escapeAlpha)
	winMgr:getWindow("sj_WndDualGame_exitCancelBtn"):setAlpha(g_escapeAlpha)
end


root:subscribeEvent("KeyUp", "OnDualGameKeyUp");
function OnDualGameKeyUp(args)	
...
--winMgr:getWindow("sj_dualgame_inputChatWindow"):setVisible(false)