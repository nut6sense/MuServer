...
function WndGame_RenderScore(deltaTime, bTeam, bItem, time, killCount, min, sec,
							 redKillCount, blueKillCount, myKillCount, myDieCount, redBumbCount, blueBumbCount, RemainLimitTime, bRenderTexture)
	...
	drawer:drawTexture("UIData/bumbmode.tga", 300, 0, 421, 103, 0, 0, WIDETYPE_5)   -- 배경
	
	-- 5초부터 카운터 표시
	local RealLimitTime = 50 - RemainLimitTime/ 60
	if RealLimitTime < 0 then
		RealLimitTime = 0
	end
	
	if RealLimitTime < 10 then
		drawer:drawTexture("UIData/bumbmode.tga", 494, 105, 41 , 52, RealLimitTime*41, 361, WIDETYPE_5) 
	end
	
	
	 -- 제한시간 게이지 바
	local BarSize = (3000 - RemainLimitTime)/8
	if BarSize < 0 then
		BarSize = 0
	end
	
	if BarSize > 60 then
		drawer:drawTexture("UIData/bumbmode.tga", 334, 85, BarSize , 13, 0, 192, WIDETYPE_5)  
	else
		if BarSize % 2 == 0 then
			drawer:drawTexture("UIData/bumbmode.tga", 334, 85, BarSize , 13, 0, 205, WIDETYPE_5)  
		else
			drawer:drawTexture("UIData/bumbmode.tga", 334, 85, BarSize , 13, 0, 218, WIDETYPE_5)  
		end
	end
	...
	if BarSize % 2 == 0 then
		if BarSize < 50 then
			drawer:drawTexture("UIData/bumbmode.tga", 330+BarSize, 73, 25 , 28, 100, 255, WIDETYPE_5)  -- 폭탄그림
		elseif BarSize < 100 then
			drawer:drawTexture("UIData/bumbmode.tga", 330+BarSize, 73, 25 , 28, 75, 255, WIDETYPE_5)  -- 폭탄그림
		elseif BarSize < 150 then
			drawer:drawTexture("UIData/bumbmode.tga", 330+BarSize, 73, 25 , 28, 50, 255, WIDETYPE_5)  -- 폭탄그림
		elseif BarSize < 200 then
			drawer:drawTexture("UIData/bumbmode.tga", 330+BarSize, 73, 25 , 28, 25, 255, WIDETYPE_5)  -- 폭탄그림
		elseif BarSize < 350 then
			drawer:drawTexture("UIData/bumbmode.tga", 330+BarSize, 73, 25 , 28, 0, 255, WIDETYPE_5)  -- 폭탄그림
		end
    else
		if BarSize < 350 then
			drawer:drawTexture("UIData/bumbmode.tga", 330+BarSize, 73, 25 , 28, 0, 255, WIDETYPE_5)  -- 폭탄그림
	...
	if bRenderTexture == 1 then
		drawer:drawTexture("UIData/BombRender1.tga", 512+134, -104, 512, 512, 0, 0, WIDETYPE_1)   -- 랜더텍스쳐 배경1
		
		local rendertick  = RemainLimitTime/6
		if rendertick % 2 == 0 then
			drawer:drawTexture("UIData/BombRender2.tga", 512+134, -104, 512, 512, 0, 0, WIDETYPE_1)   -- 렌더텍스쳐 배경2
		end
	end
	-- 분
	drawer:drawTexture("UIData/bumbmode.tga", 478, 52, 13, 19, 0, 173, WIDETYPE_5)
	drawer:drawTexture("UIData/bumbmode.tga", 493, 52, 13, 19, 0+(min*13), 173, WIDETYPE_5)
	...
	local ten = (sec/10)
	local one = (sec%10)
	drawer:drawTexture("UIData/bumbmode.tga", 516, 52, 13, 19, (ten*13), 173, WIDETYPE_5)
	drawer:drawTexture("UIData/bumbmode.tga", 532, 52, 13, 19, (one*13), 173, WIDETYPE_5)
				

	local resultValue = redBumbCount
	...
	local RedPos = 400
	if redBumbCount >= 1000 then
		RedPos = 430
		drawer:drawTexture("UIData/bumbmode.tga", RedPos, 40, 27, 35, one*27, 138, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", RedPos-27, 40, 27, 35, two*27, 138, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", RedPos-54, 40, 27, 35, three*27, 138, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", RedPos-83, 40, 27, 35, four*27, 138, WIDETYPE_5)
	elseif redBumbCount >= 100 then
		RedPos = 420
		drawer:drawTexture("UIData/bumbmode.tga", RedPos, 40, 27, 35, one*27, 138, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", RedPos-27, 40, 27, 35, two*27, 138, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", RedPos-54, 40, 27, 35, three*27, 138, WIDETYPE_5)
	elseif redBumbCount >= 10 then
		RedPos = 410
		drawer:drawTexture("UIData/bumbmode.tga", RedPos, 40, 27, 35, one*27, 138, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", RedPos-27, 40, 27, 35, two*27, 138, WIDETYPE_5)
	else
	...
	local BluePos = 600
	if blueBumbCount >= 1000 then
		BluePos = 645
		drawer:drawTexture("UIData/bumbmode.tga", BluePos, 40, 27, 35, one*27, 103, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", BluePos-27, 40, 27, 35, two*27, 103, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", BluePos-54, 40, 27, 35, three*27, 103, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", BluePos-83, 40, 27, 35, four*27, 103, WIDETYPE_5)
	elseif blueBumbCount >= 100 then
		BluePos = 625
		drawer:drawTexture("UIData/bumbmode.tga", BluePos, 40, 27, 35, one*27, 103, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", BluePos-27, 40, 27, 35, two*27, 103, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", BluePos-54, 40, 27, 35, three*27, 103, WIDETYPE_5)
	elseif blueBumbCount >= 10 then
		BluePos = 610
		drawer:drawTexture("UIData/bumbmode.tga", BluePos, 40, 27, 35, one*27, 103, WIDETYPE_5)
		drawer:drawTexture("UIData/bumbmode.tga", BluePos-27, 40, 27, 35, two*27, 103, WIDETYPE_5)
	else
...
function WndGame_NotifyPickupPropertyItem(weaponIndex, weaponPosX, weaponPosY)
	drawer:drawTexture("UIData/GameNewImage.tga", weaponPosX-40, weaponPosY-130, 130, 70, 0, 288)
end
...
----------------------------------------------------
--[[
mywindow = winMgr:createWindow("TaharezLook/Button", "sj_wndGame_movetoChannel")
...
function VisibleButton(isVisible)
	...
	winMgr:getWindow("sj_wndGame_movetoChannel"):setVisible(isVisible)
...
function WndGame_DrawObserverInfo(characterName)
	drawer:drawTexture("UIData/fightClub_005.tga", 257, 160, 514, 41, 0, 220, WIDETYPE_7)
	...
	drawer:drawTexture("UIData/quest.tga", 450, 220, 206, 36, 0, 607, WIDETYPE_7)
end


local MINIMAP_POSX = 370
local MINIMAP_POSY = 650
local MINIMAP_SIZEX = 262
local MINIMAP_SIZEY = 119

function WndGame_RenderMinimap()
	drawer:drawTextureA("UIData/bumbmode.tga", MINIMAP_POSX, MINIMAP_POSY, 262, 119, 762, 0, 255, WIDETYPE_7)
end

function WndGame_RenderCharacterPosToMap(myTeam, x, y, angle, hp, bombX, bombY)
	local posX =  x+130
	local posY = -y+49
	local texX = 497
	local texY = 0
	...
	local BombposX =  bombX+130
	local BombposY = -bombY+49
	local BombtexX = 30
	local BombtexY = 235
	local BombsizeX = 17
	local BombsizeY = 17
	...
	drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", MINIMAP_POSX+BombposX, MINIMAP_POSY+BombposY, BombsizeX, BombsizeY, BombtexX,BombtexY, 255, 255, 255, 10, 8, 0, 0, WIDETYPE_7);
	drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", MINIMAP_POSX+posX, MINIMAP_POSY+posY, sizeX, sizeY, texX, texY, 255, 255, 255, 10, 8, 0, 0, WIDETYPE_7);	
end


function WndGame_RenderMyCharacterPosToMap(myTeam, x, y, angle, hp, bombX, bombY)
	...
	sizeX = 22
	sizeY = 26
	drawer:drawTextureWithScale_Angle_Offset("UIData/fightClub_005.tga", MINIMAP_POSX+posX, MINIMAP_POSY+posY, sizeX, sizeY, texX, texY, 255, 255, 255, angle, 8, 0, 0, WIDETYPE_7);
end
...
function WndGame_NotifyBombPos(Distance, team)
	
	if bombPosbMinus == 1 then
		BombPosAlpha = BombPosAlpha - 5
	else
		BombPosAlpha = BombPosAlpha + 5
	end
	
	if BombPosAlpha <= 0 then
		BombPosAlpha = 0
		bombPosbMinus = 0
	end
	
	if BombPosAlpha >= 255  then
		BombPosAlpha = 255
		bombPosbMinus = 1
	end
	...
	if Distance > 600 then
		drawer:drawTextureA("UIData/bumbmode.tga", 0, 400, 100, 78, TexPosX, TexPosy, BombPosAlpha, WIDETYPE_2)
	end
	
	if Distance < -600 then
		drawer:drawTextureA("UIData/bumbmode.tga", 925, 400, 100, 78, TexPosX, TexPosy, BombPosAlpha, WIDETYPE_4)
	end
end
...
-- 웨폰 주울수 있는지 알려주기(폭탄전용)

----------------------------------------------------
function WndGame_NotifyPickupWeapon(weaponIndex, weaponPosX, weaponPosY)
	if weaponIndex >= 0 then
		drawer:drawTexture("UIData/bumbmode.tga", weaponPosX-70, weaponPosY-110, 150, 70, 130, 288)
	end
end


function WndGame_NotiftyUseTouch(PosX, PosY)
	drawer:drawTexture("UIData/bumbmode.tga", PosX-70, PosY-110, 130, 70, 0, 288)	
end
...
----------------------------------------------------------
local tTouchDelta = -410
local tDownDelta =  -410
local g_DownTime = { ["err"]=0, 0 }
local g_DownAlpha = 255
function WndBombGame_ClearBombEffect()
	tTouchDelta = -410
	tDownDelta = -410
	g_DownTime[1] = 0
	g_DownAlpha	= 255
...
function WndBombGame_GainPointEffect(deltaTime, point, team, effectType)
	
	
	if tTouchDelta <= 900 then
		tTouchDelta = tTouchDelta + deltaTime*2
	end
	...
	tDownDelta = tDownDelta + deltaTime
	
	g_DownAlpha = g_DownAlpha - (deltaTime/6)
	if g_DownAlpha <= 0 then
		g_DownAlpha = 0
	end
	
									
	-- 백판
	drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta,  300 , 706, 103, 0, 921, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)

	if tDownDelta < 1500 then
		-- 타입 그리기
		if effectType == 0 then
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta,  300 , 706, 65, 0, 856, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		else
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta,  300 , 706, 65, 0, 661, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		end
	else
		...
		local plusX = 0 
		if team == 0 then
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta,  300 , 706, 65, 0, 791, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		else
			plusX = 20 
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta,  300 , 706, 65, 0, 726, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		end
		
		if point == 100 then
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta+80+plusX,  300 , 156, 63, 868 , 709, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		elseif point == 80 then
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta+80+plusX,  300 , 156, 63, 868 , 772, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		elseif point == 70 then
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta+80+plusX,  300 , 156, 63, 868 , 835, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		elseif point == 60 then
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta+80+plusX,  300 , 156, 63, 868 , 898, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		else
			drawer:drawTextureWithScale_Angle_Offset("UIData/bumbmode.tga", -410+tTouchDelta+80+plusX,  300 , 156, 63, 868 , 961, 255, 255, 255, 0, 8, 100, 0,  WIDETYPE_5)
		end
	end
