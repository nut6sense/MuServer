...
function WndLobby_RenderBackImage(currentBattleChannelName)
	...
	drawer:drawTexture("UIData/LobbyImage_champ.tga", 13, 123, 662, 389, 2, 3, WIDETYPE_6)		-- 방 정보
	--drawer:drawTexture("UIData/LobbyImage_new002.tga", 705, 527, 289, 164, 0, 0, WIDETYPE_6)	-- 빠른시작
	drawer:drawTexture("UIData/LobbyImage_champ.tga", USERINFO_POSX, USERINFO_POSY+29, 323, 393, 0, 396, WIDETYPE_6)		-- 유저정보
	--drawer:drawTexture("UIData/LobbyImage_champ.tga", 411, 81, 240, 29, 353, 632, WIDETYPE_6)		-- 대전방 가기	
...
for index=0, MAX_ROOMPAGE-1 do
	...
	roomwindow:setTexture("SelectedEnabled", "UIData/LobbyImage_champ.tga", 312, 863)		-- empty(선택후 방이 없어질때를 위해)
	roomwindow:setTexture("SelectedDisabled", "UIData/LobbyImage_champ.tga", 312, 863)	-- empty(선택후 방이 없어질때를 위해)
...
function WndLobby_ExistBattleRoom(index, roomNumber, roomName, bTeam, bItem, 
	...
	if gameMode == 0 then
		if bTeam == 1 then
			winMgr:getWindow(index .. "sj_lobby_roomList_radioBtn"):setTexture("Normal", "UIData/LobbyImage_champ.tga", 0, 790)	-- 팀전
		else
			winMgr:getWindow(index .. "sj_lobby_roomList_radioBtn"):setTexture("Normal", "UIData/invisible.tga", 0, 0)			-- 개인전
		end
	elseif gameMode == 6 then
		winMgr:getWindow(index .. "sj_lobby_roomList_radioBtn"):setTexture("Normal", "UIData/LobbyImage_champ.tga", 621, 881)		-- 몬스터 레이싱
	elseif gameMode == 8 then
		winMgr:getWindow(index .. "sj_lobby_roomList_radioBtn"):setTexture("Normal", "UIData/LobbyImage_champ.tga", 665, 128)		-- 듀얼모드
	end
