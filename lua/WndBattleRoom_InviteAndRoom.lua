...
function Invite_OK()
	...
	if bSelect == true then
		...
		if IsKoreanLanguage() then
			WndBattleRoom_WarningMessageEx(inviteUserName, '님에게', '초대메세지를 보냈습니다.');
		else
...
for j=1, 2 do
	...
	elseif WndBattleRoom_GetItemMode() == 3 or WndBattleRoom_GetItemMode() == 4 or WndBattleRoom_GetItemMode() == 5 then -- 폭탄전, 미니타워전
		mywindow:setVisible(false)
	else
...
function WndBattleRoom_WarningMessageEx(errorMessage0, errorMsg1, errorMsg2)
	...
	winMgr:getWindow("sj_battleroom_error_descWindow"):addTextExtends('님에게\n', g_STRING_FONT_DODUM,115, 255,255,255,255,   0, 0,0,0,255)
